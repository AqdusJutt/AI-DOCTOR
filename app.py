...
# Load other CSV files for recommendations (update paths if needed)
...
def get_predicted_value(patient_symptoms):
    ...
    print(f"Input Vector: {input_vector}")  # Debugging line
    pred_idx = svc.predict([input_vector])[0]
    print(f"Predicted Index: {pred_idx}")  # Debugging line
    return diseases_list.get(pred_idx, "Disease not found")

@app.route('/', methods=['GET', 'POST'])
def home():
    return render_template('home.html')  # Render the home page first

@app.route('/index', methods=['GET'])
def index():
    return render_template('index.html')  # Render the index page for symptoms input
...
def predict():
    ...
    return render_template('result.html', disease=disease, desc=desc, precautions=pre, meds=med, diets=die, workouts=wrkout)
