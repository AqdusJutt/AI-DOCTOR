...
This system predicts diseases based on symptoms using multiple ML models,
then recommends medicines, precautions, workouts, diets, and descriptions.
"""
...
base_path = r'C:\Users\hp\Desktop\ML&AI Project'  # Modify for your environment

dataset = pd.read_csv(os.path.join(base_path, 'Training.csv'))
sym_des = pd.read_csv(os.path.join(base_path, "Symptom-severity.csv"))
...
# Train and save best model (SVC here)
svc = SVC(kernel='linear')
...
# Symptom dictionary (symptom to index)
symptoms_dict = {
...
def helper(dis):
    ...
    pre = []
    if not pre_df.empty:
        pre = pre_df[['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values[0].tolist()
...
# Prediction function using svc model
def get_predicted_value(patient_symptoms):
    ...
    return diseases_list.get(pred_index, "Disease not found")

# Medicine recommendation system
def recommend_medicine(disease):
    recommendations = {
        'Fungal infection': ['Antifungal cream', 'Ketoconazole'],
        'Allergy': ['Cetirizine', 'Antihistamines'],
        'Diabetes': ['Insulin', 'Metformin'],
        'Hypertension': ['Amlodipine', 'Beta blockers'],
        'Common Cold': ['Paracetamol', 'Rest', 'Hydration'],
        'Migraine': ['Ibuprofen', 'Sumatriptan'],
        'Heart attack': ['Aspirin', 'Nitroglycerin', 'Emergency care'],
        'Hypothyroidism': ['Levothyroxine'],
        'Hyperthyroidism': ['Methimazole'],
        'Peptic ulcer disease': ['Omeprazole', 'Antacids']
    }
    return recommendations.get(disease, ['Consult a physician for detailed diagnosis and treatment.'])

# Simple NLP-based symptom processing
def process_symptoms(input_text):
    ...
    processed = {col: 0 for col in dataset.columns[:-1]}
...
if __name__ == "__main__":
    # Direct user input of symptoms for ML prediction + detailed info
    ...
    for i, p in enumerate(pre, 1):
        print(f"{i}. {p}")
    print("\n===== Medicines =====")
    meds = recommend_medicine(predicted_disease)
    for m in meds:
        print(f"- {m}")
